...
cMojangAPI::cMojangAPI():
	...
	m_FasterCache(new FasterDB::cFasterMojangCache("MojangAPI"))
{
}
...
cUUID cMojangAPI::GetUUIDFromPlayerName(const AString & a_PlayerName, bool a_UseOnlyCached)
{
	...
	// Check FASTER cache if enabled:
	if (m_FasterCache != nullptr)
	{
		cUUID UUID;
		if (m_FasterCache->GetUUIDByName(lcPlayerName, UUID))
		{
			return UUID;
		}
	}
...
AString cMojangAPI::GetPlayerNameFromUUID(const cUUID & a_UUID, bool a_UseOnlyCached)
{
	...
	if (m_FasterCache != nullptr)
	{
		AString PlayerName;
		Int64 DateTime;
		if (m_FasterCache->GetProfile(a_UUID, PlayerName, DateTime))
		{
			return PlayerName;
		}
	}
...
void cMojangAPI::AddPlayerNameToUUIDMapping(const AString & a_PlayerName, const cUUID & a_UUID)
{
	...
	if (m_FasterCache != nullptr)
	{
		m_FasterCache->PutNameToUUID(a_PlayerName, a_UUID);
	}
...
void cMojangAPI::AddPlayerProfile(const AString & a_PlayerName, const cUUID & a_UUID, const Json::Value & a_Properties)
{
	...
	{
		m_FasterCache->PutNameToUUID(a_PlayerName, a_UUID);
		m_FasterCache->PutProfile(a_UUID, a_PlayerName, Now);
	}
	NotifyNameUUID(a_PlayerName, a_UUID);
}
...
{
	// FASTER handles persistence automatically via its hybrid log.
	// We could optionally load an existing checkpoint here.
}
...
{
	// FASTER persistence is managed via its log and optional checkpoints.
	// In a real 'mesh' system, we'd handle checkpoints periodically.
}
